...
export function FamilySelectionSummary({ heirs, onClear, language = "ta" }: FamilySelectionSummaryProps) {
  ...
  return (
    <aside className="family-guide mt-5 rounded-2xl border border-blue-200 bg-white p-4 shadow-sm lg:sticky lg:top-6 lg:mt-0" aria-label={language === "en" ? "Selected family members" : "தேர்ந்தெடுத்த குடும்ப உறுப்பினர்கள்"}>
      ...
      {selected.length === 0 ? <p className="mt-4 rounded-xl bg-slate-50 px-3 py-3 text-sm leading-6 text-slate-500">{language === "en" ? "No family member has been selected yet." : "இன்னும் யாரும் தேர்வு செய்யப்படவில்லை."}</p> : <div className="mt-4 max-h-[52vh] space-y-4 overflow-y-auto pr-1">{selectedCore.length > 0 ? <section><p className="mb-2 text-[11px] font-extrabold uppercase tracking-wide text-slate-400">{language === "en" ? "Close family" : "நெருங்கிய குடும்பம்"}</p><div className="space-y-2">{selectedCore.map((item) => <div key={item.key} className="flex items-center gap-2 rounded-xl bg-blue-50 px-2.5 py-2"><span aria-hidden="true">{item.emoji}</span><p className="min-w-0 flex-1 text-xs font-bold leading-5 text-slate-800">{item.displayLabel} <span className="text-[#133D76]">· {item.count}</span></p><button type="button" onClick={() => onClear(item.key)} className="grid size-8 place-items-center rounded-lg text-slate-400 hover:bg-white hover:text-rose-700" aria-label={language === "en" ? `Remove ${item.displayLabel}` : `${item.displayLabel} அகற்ற`}><X size={15} /></button></div>)}</div></section> : null}{selectedExtended.length > 0 ? <section><p className="mb-2 text-[11px] font-extrabold uppercase tracking-wide text-slate-400">{language === "en" ? "Other book relatives" : "புத்தகத்தில் உள்ள மற்ற உறவுகள்"}</p><div className="space-y-2">{selectedExtended.map((item) => <div key={item.key} className="flex items-center gap-2 rounded-xl bg-blue-50 px-2.5 py-2"><span aria-hidden="true">{item.emoji}</span><p className="min-w-0 flex-1 text-xs font-bold leading-5 text-slate-800">{item.displayLabel} <span className="text-[#133D76]">· {item.count}</span></p><button type="button" onClick={() => onClear(item.key)} className="grid size-8 place-items-center rounded-lg text-slate-400 hover:bg-white hover:text-rose-700" aria-label={language === "en" ? `Remove ${item.displayLabel}` : `${item.displayLabel} அகற்ற`}><X size={15} /></button></div>)}</div></section> : null}</div>}
